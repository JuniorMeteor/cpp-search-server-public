...
int main() {
    ...
    const auto documents = GenerateQueries(generator, dictionary, 1000, 20); // (generator, dictionary, 10'000, 70);
    SearchServer search_server(dictionary[0]);
    for (size_t i = 0; i < documents.size(); ++i) {
        search_server.AddDocument(i, documents[i], DocumentStatus::ACTUAL, { 1, 2, 3 });
    }
    const auto queries = GenerateQueries(generator, dictionary, 100, 20); // (generator, dictionary, 100, 70);
    TEST(seq);
    TEST(par);
